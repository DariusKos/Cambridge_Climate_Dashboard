...
def build_annual_overview(air, rain, ground):
    """Build the annual-overview metrics with a SEASON dimension.

    Every base metric (air mean/max/min, ground 30/100 cm, rainfall) is
    computed five ways: for the whole year ('annual') and for each
    meteorological season (winter DJF, spring MAM, summer JJA, autumn SON).
    December is assigned to the FOLLOWING year's winter so each winter is
    kept together (Dec 2020 + Jan/Feb 2021 → winter 2021).

    Temperatures are seasonal/annual *means*; rainfall is the *total*.

    Metrics are keyed '{base}__{season}', e.g. 'air_tmean__summer'. The HTML
    exposes a season toggle plus the base-metric buttons.
    """
    ...
    # Minimum obs required per season (~same proportion as the annual threshold)
    SEASON_MIN_OBS = int(MIN_OBS_PER_YEAR * 90 / 365)

    def season_year(dates, months):
        """Return a Series mapping each row to its season-year. For winter,
        December belongs to the next calendar year."""
        yr = dates.dt.year.copy()
        mo = dates.dt.month
        yr = yr.where(mo != 12, yr + 1)  # Dec → next year
        return yr

    # ---- prepare per-metric, per-season aggregates ----
    air = air.copy()
    rain = rain.copy()
    ...
    def add_metric(key, label, desc, unit, dp, color, edge, group,
                   years, values, incomplete, n_obs, min_obs):
        valid_mask = ~np.asarray(incomplete, dtype=bool)
        metrics[key] = {
            ...
            'values': [None if (v is None or (isinstance(v, float) and np.isnan(v)))
                       else round(float(v), dp) for v in values],
            'incomplete': [bool(x) for x in incomplete],
            'n_obs': [int(x) for x in n_obs],
            'min_obs': int(min_obs),
            'fit': fit_with_mask(fit_ols,
                                 np.asarray(years, dtype=float),
                                 np.asarray(values, dtype=float),
                                 valid_mask),
            **basic_stats(np.asarray(years), np.asarray(values, dtype=float),
                          dp=dp, valid_mask=valid_mask),
        }

    # ===== AIR TEMPERATURE (mean of tmax, mean of tmin, tmean) =====
    air_defs = [
        ('air_tmax',  'max',  'avg daily max', '#d97559', '#9c4528'),
        ('air_tmin',  'min',  'avg daily min', '#c98c6a', '#7a4a32'),
        ('air_tmean', 'mean', 'annual mean',   '#d97559', '#9c4528'),
    ]
    for season in SEASONS:
        if season == 'annual':
            ...
            sub = air[air['month'].isin(SEASON_MONTHS[season])]
            sy = season_year(sub['date'], sub['month'])
            min_obs = SEASON_MIN_OBS
        tmp = sub.assign(_sy=sy.values)
        agg = (tmp.groupby('_sy')
                  .agg(tmax=('max_air_temp', 'mean'),
                       tmin=('min_air_temp', 'mean'),
                       n_max=('max_air_temp', 'count'),
                       n_min=('min_air_temp', 'count'))
                  .reset_index().rename(columns={'_sy': 'year'}))
        agg['tmean'] = (agg['tmax'] + agg['tmin']) / 2
        yrs = agg['year'].values
        for base_key, col, _, color, edge in air_defs:
            if col == 'mean':
                vals = agg['tmean'].values
                n_obs = agg[['n_max', 'n_min']].min(axis=1).astype(int).values
                inc = (agg['n_max'] < min_obs) | (agg['n_min'] < min_obs)
            elif col == 'max':
                vals = agg['tmax'].values
                n_obs = agg['n_max'].astype(int).values
                inc = agg['n_max'] < min_obs
            else:
                vals = agg['tmin'].values
                n_obs = agg['n_min'].astype(int).values
                inc = agg['n_min'] < min_obs
            season_word = season.capitalize() if season != 'annual' else 'Annual'
            label = f'Air: {season_word.lower()} ' + \
                    {'air_tmax': 'mean daily max',
                     'air_tmin': 'mean daily min',
                     'air_tmean': 'mean temperature'}[base_key]
            desc = (f'{season_word} mean of the daily '
                    + {'air_tmax': 'maximum', 'air_tmin': 'minimum',
                       'air_tmean': '(max+min)/2'}[base_key]
                    + ' air temperature.'
                    + (' December is grouped with the following year\'s winter.'
                       if season == 'winter' else ''))
            add_metric(f'{base_key}__{season}', label, desc, '°C', 1,
                       color, edge, 'Air temperature',
                       yrs, vals, inc.values, n_obs, min_obs)

    # ===== GROUND TEMPERATURE (mean soil temp per depth) =====
    for label_g, df_g in ground.items():
        depth = '30' if '30' in label_g else '100'
        colors = ('#7e1e9c', '#4a1058') if depth == '30' else ('#5e3c8e', '#3d2462')
        dg = df_g.copy()
        dg['date'] = pd.to_datetime(dg['date'])
        dg['month'] = dg['date'].dt.month
        for season in SEASONS:
            if season == 'annual':
                sub = dg; sy = dg['date'].dt.year; min_obs = MIN_OBS_PER_YEAR
            else:
                sub = dg[dg['month'].isin(SEASON_MONTHS[season])]
                sy = season_year(sub['date'], sub['month']); min_obs = SEASON_MIN_OBS
            tmp = sub.assign(_sy=sy.values)
            agg = (tmp.groupby('_sy')
                      .agg(value=('value', 'mean'), n_obs=('value', 'count'))
                      .reset_index().rename(columns={'_sy': 'year'}))
            inc = agg['n_obs'] < min_obs
            season_word = season.capitalize() if season != 'annual' else 'Annual'
            base_key = f'ground_bg{depth}cm'
            label = f'Ground {depth} cm: {season_word.lower()} mean'
            desc = (f'{season_word} mean soil temperature at Cambridge Botanic '
                    f'Garden, {depth} cm depth (1911–2019).'
                    + (' Note the autumn lag: deep soil stays warm well into '
                       'autumn because it releases summer heat slowly.'
                       if (depth == '100' and season == 'autumn') else ''))
            add_metric(f'{base_key}__{season}', label, desc, '°C', 1,
                       colors[0], colors[1], 'Ground temperature',
                       agg['year'].values, agg['value'].values,
                       inc.values, agg['n_obs'].astype(int).values, min_obs)

    # ===== RAINFALL (seasonal/annual TOTAL) =====
    for season in SEASONS:
        if season == 'annual':
            sub = rain; sy = rain['year']; min_obs = MIN_OBS_PER_YEAR
        else:
            sub = rain[rain['month'].isin(SEASON_MONTHS[season])]
            sy = season_year(sub['date'], sub['month']); min_obs = SEASON_MIN_OBS
        tmp = sub.assign(_sy=sy.values)
        agg = (tmp.groupby('_sy')
                  .agg(total=('rain', 'sum'), n_obs=('rain', 'count'))
                  .reset_index().rename(columns={'_sy': 'year'}))
        inc = agg['n_obs'] < min_obs
        season_word = season.capitalize() if season != 'annual' else 'Annual'
        label = f'Rainfall: {season_word.lower()} total'
        desc = (f'{season_word} total rainfall (mm).'
                + (' December is grouped with the following year\'s winter.'
                   if season == 'winter' else '')
                + (' Cambridge\'s wettest season.' if season == 'winter' else '')
                + (' Despite expectations of drier UK summers, the historical '
                   'trend here is essentially flat.' if season == 'summer' else ''))
        add_metric(f'rain_total__{season}', label, desc, 'mm', 0,
                   '#3a7ca5', '#1f5879', 'Rainfall',
                   agg['year'].values, agg['total'].values,
                   inc.values, agg['n_obs'].astype(int).values, min_obs)

    groups = {
        'Air temperature':    ['air_tmean', 'air_tmax', 'air_tmin'],
        'Ground temperature': ['ground_bg30cm', 'ground_bg100cm'],
        'Rainfall':           ['rain_total'],
    }
    return {'metrics': metrics, 'groups': groups, 'seasons': SEASONS}
...
<div class="container">
  ...
  <div class="panel active" id="panel-overview">
    <div class="controls">
      ...
      <button class="btn active" data-season="annual">Annual</button>
      <button class="btn" data-season="winter">Winter (DJF)</button>
      <button class="btn" data-season="spring">Spring (MAM)</button>
      <button class="btn" data-season="summer">Summer (JJA)</button>
      <button class="btn" data-season="autumn">Autumn (SON)</button>
    </div>
    ...
    <div class="note">
      Pick a <b>season</b> and a <b>metric</b> to see that metric for that part
      of the year across the whole record. Temperatures are seasonal means;
      rainfall is the seasonal total. Winter groups December with the
      following January and February. This lets you compare, for example,
      how summer maximum temperatures have risen versus winter minimums, or
      whether deep-soil temperature shifts more in one season than another.
      <br><br>
      ...
      <br><br>
      <b>Incomplete years</b> (fewer than 340 daily obs for the year, or ~84
      for a single season) are shown in striped grey: their values are drawn
      so gaps are visible, but they're excluded from the trend fit, baseline
      mean, 5-year rolling mean, and high/low stats. Hover any bar for its
      observation count.
      <br><br>
      <b>Ground temperature</b> is Cambridge Botanic Garden, 30 cm and 100 cm
...
// Buttons select a BASE metric (e.g. 'air_tmean'); the season toggle picks
// which seasonal variant ('air_tmean__summer') is actually rendered.
let currentOverviewBase = 'air_tmean';
let currentSeason = 'annual';

// Short, season-independent labels for the base-metric buttons.
const OVERVIEW_BASE_LABELS = {
  air_tmean:      'Air: mean',
  air_tmax:       'Air: daily max',
  air_tmin:       'Air: daily min',
  ground_bg30cm:  'Ground: 30 cm',
  ground_bg100cm: 'Ground: 100 cm',
  rain_total:     'Rainfall total',
};
...
(function() {
  ...
  for (const [group, keys] of Object.entries(overviewBundle.groups)) {
    ...
    keys.forEach(baseKey => {
      ...
      b.className = 'btn'; b.dataset.base = baseKey;
      b.textContent = OVERVIEW_BASE_LABELS[baseKey] || baseKey;
      b.addEventListener('click', () => {
        currentOverviewBase = baseKey; renderOverview();
      });
      row.appendChild(b);
    });
  ...
  // Season toggle
  document.querySelectorAll('#panel-overview .btn[data-season]').forEach(b =>
    b.addEventListener('click', () => {
      currentSeason = b.dataset.season; renderOverview();
    }));
})();

function renderOverview() {
  // Resolve the base metric + season to a concrete metric key. Fall back to
  // annual if a particular season variant somehow doesn't exist.
  let key = currentOverviewBase + '__' + currentSeason;
  if (!overviewBundle.metrics[key]) key = currentOverviewBase + '__annual';
  renderBarTab(overviewBundle, 'chart-overview', 'stats-overview',
               'overview-desc', key);
  document.querySelectorAll('#overview-buttons .btn').forEach(b =>
    b.classList.toggle('active', b.dataset.base === currentOverviewBase));
  document.querySelectorAll('#panel-overview .btn[data-season]').forEach(b =>
